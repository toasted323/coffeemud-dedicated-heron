		...
		while(!ok)
		{
			...
			for(int i=0;i<statCodes.length;i++)
			{
				...
				if((showFlag!=showNumber)&&(showFlag>-999))
					continue;
				final String res=mob.session().getSyncModalDialogManager().choose(L("D)atabase Value, E)dit Value, or N)o Change, or Q)uit All: "),L("DENQ"), L("N"));
			...
			synchronized(CMClass.getSync("SYNC"+dbR.roomID()))
			{
				...
				for(final MOB dbM : mobSet)
				{
					...
					if(M==null)
					{
						...
						{
							if(mob.session().getSyncModalDialogManager().confirm(L("MOB: @x1.@x2 not in local room.\n\rWould you like to add it (y/N)?",dbR.roomID(),rName), L("N")))
							{
								M=(MOB)dbM.copyOf();
					...
					else
					{
						...
						for(final Item dbI : itemSet)
						{
							...
							if(I==null)
							{
								...
								{
									if(mob.session().getSyncModalDialogManager().confirm(L("Item: @x1.@x2.@x3 not in local room.\n\rWould you like to add it (y/N)?",dbR.roomID(),dbM.Name(),rIName), L("N")))
									{
										I=(Item)dbI.copyOf();
										M.addItem(I);
										doneI.add(I);
						...
						for(final Enumeration<Item> i=M.items();i.hasMoreElements();)
						{
							...
							{
								if(mob.session().getSyncModalDialogManager().confirm(L("Item: @x1.@x2.@x3 not in database.\n\rWould you like to delete it (y/N)?",R.roomID(),M.Name(),I.Name()), L("N")))
								{
									M.delItem(I);
									updateMobs=true;
				...
				for(final Enumeration<MOB> r=R.inhabitants();r.hasMoreElements();)
				{
					...
					{
						if(mob.session().getSyncModalDialogManager().confirm(L("MOB: @x1.@x2 not in database.\n\rWould you like to delete it (y/N)?",R.roomID(),M.Name()), L("N")))
						{
							R.delInhabitant(M);
							updateMobs=true;
				...
				for(final Item dbI : itemSet)
				{
					...
					if(I==null)
					{
						if(amMerging(doType,mask,dbI)&&(!ignore.contains("MISSING")))
						{
							if(mob.session().getSyncModalDialogManager().confirm(L("Item: @x1.@x2 not in local room.\n\rWould you like to add it (y/N)?",dbR.roomID(),rName), L("N")))
							{
								I=(Item)dbI.copyOf();
								R.addItem(I);
								doneI.add(I);
				...
				for(final Enumeration<Item> i=R.items();i.hasMoreElements();)
				{
					...
					if(amMerging(doType,mask,I)&&(!doneI.contains(I))&&(!ignore.contains("EXTRA")))
					{
						if(mob.session().getSyncModalDialogManager().confirm(L("Item: @x1.@x2 not in database.\n\rWould you like to delete it (y/N)?",R.roomID(),I.Name()), L("N")))
						{
							R.delItem(I);
							updateItems=true;
